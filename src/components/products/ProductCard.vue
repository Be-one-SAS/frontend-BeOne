<template>
  <div
    ...
    :class="disponible ? 'cursor-pointer' : 'cursor-not-allowed'" @click="disponible && handleSelect()">
    <!-- Overlay no disponible -->
    <div v-if="!disponible" class="absolute inset-0 bg-white/70 flex items-center justify-center z-10">
      <span
        class="text-[12px] font-semibold text-text-2 bg-white/90 px-3 py-1 rounded-full border border-border shadow-sm">
        No disponible
      </span>
    </div>
    ...
    <div class="w-full aspect-[4/3] overflow-hidden bg-border-light">
      <img :src="imageUrl" @error="onImageError" class="w-full h-full object-cover" alt="producto" />
    </div>
